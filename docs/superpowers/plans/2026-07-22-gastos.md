...
- `pagado` = 654800 y `pendiente` = 20000.
...
// Las claves TIENEN que ser propiedades CSS reales. `tabla-campanas.tsx` usa `bg`/`c`,
// que React descarta en silencio: el badge queda sin color y el semáforo no se ve.
...
2. **Proveedor del merch.** Las chombas y los buzos se pagaron el 22/07/2026 (confirmado por Fabricio); falta cargar de qué proveedor.
