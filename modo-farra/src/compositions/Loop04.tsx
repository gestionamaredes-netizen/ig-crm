...
const Loop04: React.FC = () => {
  const loopVariation = useLoopVariation(4);
  ...
  return (
    <div style={{ width: '100%', height: '100%', backgroundColor: COLORS.black, position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Loop 4: Previa/Boliche/Noche focus */}
      <Sequence from={TIMECODES.MATERIAL_START} durationInFrames={300}>
        <MemoryScene from={TIMECODES.MATERIAL_START} duration={300} type="commercial" />
      </Sequence>
      ...
      <Sequence from={TIMECODES.MATERIAL_START + 600} durationInFrames={300}>
        <MemoryScene from={TIMECODES.MATERIAL_START + 600} duration={300} type="music" />
      </Sequence>
      ...
      <Sequence from={TIMECODES.MATERIAL_START + 2100} durationInFrames={240}>
        <TVScene from={TIMECODES.MATERIAL_START + 2100} duration={240} channels={['LA ÚLTIMA', 'HASTA LA 1 GRATIS', 'NO SE VA NADIE', 'AFUERA HAY COLA']} />
      </Sequence>
      ...
      <Sequence from={TIMECODES.PEAK_FRAME} durationInFrames={100}>
        <div style={{ width: '100%', height: '100%', backgroundColor: COLORS.black, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <CRT preset="BROKEN_SIGNAL" intensity="heavy">
            <GiantText text="ESTO RECIÉN EMPIEZA" fontSize={loopVariation.peakVariation.fontSize} color={COLORS.whiteFlash} />
          </CRT>
        </div>
      </Sequence>
