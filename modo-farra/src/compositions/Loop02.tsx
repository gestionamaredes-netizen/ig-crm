...
const Loop02: React.FC = () => {
  const loopVariation = useLoopVariation(2);
  ...
  return (
    ...
    >
      ...
      {/* MATERIAL BLOCKS: MSN/Digital focused */}
      <Sequence from={TIMECODES.MATERIAL_START} durationInFrames={300}>
        <WindowScene from={TIMECODES.MATERIAL_START} duration={300} title="MSN MESSENGER" content="¿ESTÁS EN MSN?" />
      </Sequence>
      ...
      <Sequence from={TIMECODES.MATERIAL_START + 600} durationInFrames={300}>
        <WindowScene from={TIMECODES.MATERIAL_START + 600} duration={300} title="CONECTANDO..." content="ESTÁ ESCRIBIENDO..." />
      </Sequence>
      ...
      <Sequence from={TIMECODES.MATERIAL_START + 1500} durationInFrames={300}>
        <WindowScene from={TIMECODES.MATERIAL_START + 1500} duration={300} title="SISTEMA" content="ERROR - NO RESPONDE" />
      </Sequence>

      <Sequence from={TIMECODES.MATERIAL_START + 1800} durationInFrames={300}>
        <MemoryScene from={TIMECODES.MATERIAL_START + 1800} duration={300} type="commercial" />
      </Sequence>

      <Sequence from={TIMECODES.MATERIAL_START + 2100} durationInFrames={240}>
        <TVScene from={TIMECODES.MATERIAL_START + 2100} duration={240} channels={['FOTOLOG', 'MESSENGER', 'CHAT', 'CONECTANDO']} />
      </Sequence>
      ...
      <Sequence from={TIMECODES.PEAK_FRAME} durationInFrames={100}>
        <div style={{ width: '100%', height: '100%', backgroundColor: COLORS.black, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <CRT preset="BROKEN_SIGNAL" intensity="heavy">
            <GiantText text="CONECTANDO..." fontSize={loopVariation.peakVariation.fontSize} color={COLORS.whiteFlash} />
          </CRT>
        </div>
      </Sequence>
